...
--    end if;
  ...
  function pp_clob(obj json, v_path varchar2) return clob as
    json_part json_value;
    ret_clob CLOB;
  begin
    json_part := json_ext.get_json_value(obj, v_path);
    if(json_part is null) then 
      ...
      dbms_lob.createtemporary(ret_clob, true);       
      json_printer.pretty_print_any ( json_part => json_part
                                    , buf       => ret_clob ); --escapes a possible internal string
      return ret_clob;
    end if;       
  end pp_clob;
