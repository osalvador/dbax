...
--
  ...
  begin
    ...
    t_tmp := XMLTYPE(p_xml).getblobval(NLS_CHARSET_ID ('AL32UTF8')); 
...
<calcPr calcId="144525"/>
    ...
    t_xxx := '<?xml version="1.0" encoding="ISO" standalone="yes"?>
...
--
  ...
  exception
    ...
    /**
    --New functionality
    --Finally call finish to retrieve de BLOB
    */
    ...
    PROCEDURE query2sheet (p_cursor            PLS_INTEGER
                         , p_column_headers    BOOLEAN := TRUE
                         , p_sheet             PLS_INTEGER := NULL)
    AS
       t_sheet       PLS_INTEGER;
    ...
    BEGIN
       ...
       LOOP
          ...
          THEN
             cell (p_col       => c
                 , p_row       => 1
                 , p_value     => t_desc_tab (c).col_name
                 , p_sheet     => t_sheet
                 , p_borderid  => xlsx_builder_pkg.get_border ('medium'
                                                             , 'medium'
                                                             , 'medium'
                                                             , 'medium')
                 , p_fillid    => xlsx_builder_pkg.get_fill ('solid', 'ADA1AA')
                 , p_fontid    => xlsx_builder_pkg.get_font (p_name => 'ARIAL', p_bold => TRUE));
                 
                 set_column_width (c, (length(t_desc_tab (c).col_name) + 5 ) * 1.3);
          /*cell (c
              , 1
              , t_desc_tab (c).col_name
              , p_sheet     => t_sheet);*/
          ...
          CASE
             ...
             THEN
                DBMS_SQL.define_array (t_c
                                     ...
                                     , t_bulk_size
                                     , 1);
             WHEN t_desc_tab (c).col_type IN (1, 8, 9, 96)
             THEN
                DBMS_SQL.define_array (t_c
                                     , c
                                     , v_tab
                                     , t_bulk_size
                                     , 1);
             WHEN t_desc_tab (c).col_type IN (112)
             THEN
                DBMS_SQL.define_array (t_c
                                     , c
                                     , c_tab
             ...
             LOOP
                CASE
                   ...
                   THEN
                      ...
                      v_tab.delete;
                   WHEN t_desc_tab (c).col_type IN (112) --CLOB
                   THEN
                      DBMS_SQL.COLUMN_VALUE (t_c, c, c_tab);

                      FOR i IN 0 .. t_r - 1
                      LOOP
                         cell (c
                             , t_cur_row + i
                             , dbms_lob.substr(c_tab (i + c_tab.FIRST ()))                             
                             , p_sheet     => t_sheet);
                      END LOOP;
