...
IS
   ...
   */
   ...
   e_del_failed      EXCEPTION; --The delete operation failed
    ...
    --    |p_<%=  pk(i).COLUMN_NAME%> | ${table_name}.<%=pk(i).COLUMN_NAME %>%TYPE | must be NOT NULL
    <% end loop; %>
    --### Amendments
    --| When         | Who                      | What
    --|--------------|--------------------------|------------------
...
DECLARE
   p_tapi_applications   CLOB;
   p_tapi_employees     CLOB;
   p_vars               teplsql.t_assoc_array;
BEGIN
   p_vars ('table_name') := 'WDX_VIEWS';
    p_vars ('date') := TO_CHAR (SYSDATE, 'DD-MON-YYYY HH24:MI');
    p_vars ('user') := USER;
    p_vars ('result_cache') := 'RESULT_CACHE';

   p_tapi_applications := teplsql.process ('tapi_gen2_body', p_vars, 'body');
   show_clob(p_tapi_applications);

   /*p_vars ('table_name') := 'OG_EMPLOYEES';
   p_tapi_employees := teplsql.process ('test_tmpl', p_vars);
   DBMS_OUTPUT.put_line (p_tapi_employees);*/
END;
