--
-- TAPI_WDX_VIEWS  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY      tapi_wdx_views IS

   /**
   -- # TAPI_wdx_views
   -- Generated by: tapiGen2 - DO NOT MODIFY!
   -- Website: github.com/osalvador/tapiGen2
   -- Created On: 05-AGO-2015 16:18
   -- Created By: DBAX
   */

    

   --GLOBAL_PRIVATE_CURSORS
   --By PK
   CURSOR wdx_views_cur (      
        p_appid     IN      wdx_views.appid%TYPE,
		p_name     IN      wdx_views.name%TYPE
   )
   IS
      SELECT appid
           , name
           , title
           , source
           , description
           , visible
           , created_by
           , created_date
           , modified_by
           , modified_date
           , tapi_wdx_views.hash(appid, name)
           , ROWID
      FROM wdx_views
      WHERE appid = UPPER(wdx_views_cur.p_appid) AND 
            name = wdx_views_cur.p_name
      FOR UPDATE;

    --By Rowid
    CURSOR wdx_views_rowid_cur (p_rowid     IN      varchar2)
    IS
      SELECT appid
           , name
           , title
           , source
           , description
           , visible
           , created_by
           , created_date
           , modified_by
           , modified_date
           , tapi_wdx_views.hash(appid, name)
           , ROWID
      FROM wdx_views
      WHERE ROWID = p_rowid
      FOR UPDATE;    


    FUNCTION hash (
        p_appid     IN      wdx_views.appid%TYPE,
		p_name     IN      wdx_views.name%TYPE
          )
      RETURN varchar2
   IS
         
      
      l_retval hash_t;
      l_string CLOB;
      l_date_format varchar2(64);
   BEGIN       

      
            
      --Get actual NLS_DATE_FORMAT
      SELECT VALUE INTO l_date_format  FROM v$nls_parameters WHERE parameter ='NLS_DATE_FORMAT';
      --Alter session for date columns
      EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_DATE_FORMAT=''YYYY/MM/DD hh24:mi:ss''';
      
      SELECT  appid ||
               name ||
               title ||
               source ||
               description ||
               visible ||
               created_by ||
               created_date ||
               modified_by ||
               modified_date 
      INTO l_string
      FROM wdx_views
      WHERE  appid =  UPPER(hash.p_appid) AND 
             name =  hash.p_name;

      --Restore NLS_DATE_FORMAT to default
      EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_DATE_FORMAT=''' || l_date_format|| '''';
              
      
      l_retval := DBMS_CRYPTO.hash(l_string, DBMS_CRYPTO.hash_sh1);

      
      RETURN l_retval;
   
   
   END hash;
   
    FUNCTION hash_rowid (p_rowid IN varchar2)
      RETURN varchar2
   IS
          
      l_retval hash_t;
      l_string CLOB;
      l_date_format varchar2(64);      
   BEGIN
      

      --Get actual NLS_DATE_FORMAT
      SELECT VALUE INTO l_date_format  FROM v$nls_parameters WHERE parameter ='NLS_DATE_FORMAT';
      --Alter session for date columns
      EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_DATE_FORMAT=''YYYY/MM/DD hh24:mi:ss''';

      SELECT  appid ||
               name ||
               title ||
               source ||
               description ||
               visible ||
               created_by ||
               created_date ||
               modified_by ||
               modified_date 
      INTO l_string
      FROM wdx_views
      WHERE  ROWID = p_rowid;

      --Restore NLS_DATE_FORMAT to default
      EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_DATE_FORMAT=''' || l_date_format|| '''';
      
      
      l_retval := DBMS_CRYPTO.hash(l_string, DBMS_CRYPTO.hash_sh1);

      
      RETURN l_retval;
   
   
   END hash_rowid;   

   FUNCTION rt (
        p_appid     IN      wdx_views.appid%TYPE,
		p_name     IN      wdx_views.name%TYPE
          )
      RETURN wdx_views_rt 
   IS
        
      l_wdx_views_rec wdx_views_rt;
   BEGIN
          

      SELECT a.*, tapi_wdx_views.hash(appid, name), rowid
      INTO l_wdx_views_rec
      FROM wdx_views a
      WHERE appid= UPPER(rt.p_appid) AND 
            name= rt.p_name;

            
      RETURN l_wdx_views_rec;

   
   END rt;

   FUNCTION rt_for_update (
        p_appid     IN      wdx_views.appid%TYPE,
		p_name     IN      wdx_views.name%TYPE
          )
      RETURN wdx_views_rt 
   IS
            
      l_wdx_views_rec wdx_views_rt;
   BEGIN

      
      
      SELECT a.*, tapi_wdx_views.hash(appid, name), rowid
      INTO l_wdx_views_rec
      FROM wdx_views a
      WHERE appid= UPPER(rt_for_update.p_appid) AND 
            name= rt_for_update.p_name
      FOR UPDATE;

      
      RETURN l_wdx_views_rec;
   
   
   END rt_for_update;

    FUNCTION tt (
        p_appid IN wdx_views.appid%TYPE DEFAULT NULL,
		p_name IN wdx_views.name%TYPE DEFAULT NULL
              )
       RETURN wdx_views_tt
       PIPELINED
    IS
            
       l_wdx_views_rec   wdx_views_rt;
    BEGIN

       
      
       FOR c1 IN (SELECT   a.*, ROWID
                    FROM   wdx_views a
                   WHERE appid= NVL(UPPER(TT.p_appid),appid) AND 
                         name= NVL(TT.p_name,name))                   
       LOOP
              l_wdx_views_rec.appid := c1.appid;
              l_wdx_views_rec.name := c1.name;
              l_wdx_views_rec.title := c1.title;
              l_wdx_views_rec.source := c1.source;
              l_wdx_views_rec.description := c1.description;
              l_wdx_views_rec.visible := c1.visible;
              l_wdx_views_rec.created_by := c1.created_by;
              l_wdx_views_rec.created_date := c1.created_date;
              l_wdx_views_rec.modified_by := c1.modified_by;
              l_wdx_views_rec.modified_date := c1.modified_date;
              
              l_wdx_views_rec.hash := tapi_wdx_views.hash(c1.appid, c1.name);
              l_wdx_views_rec.row_id := c1.ROWID;
              PIPE ROW (l_wdx_views_rec);
       END LOOP;

              
       RETURN;
    
    
    END tt;


    PROCEDURE ins (p_wdx_views_rec IN OUT wdx_views_rt)
    IS
      
             
       l_rowtype     wdx_views%ROWTYPE;       
       l_user_name   wdx_views.CREATED_BY%TYPE := NVL(dbax_core.g$username,USER);
       l_date        wdx_views.CREATED_DATE%TYPE := SYSDATE;
              
    BEGIN
       
      
       p_wdx_views_rec.CREATED_BY := l_user_name;
       p_wdx_views_rec.CREATED_DATE := l_date;
       p_wdx_views_rec.MODIFIED_BY := l_user_name;
       p_wdx_views_rec.MODIFIED_DATE := l_date;
       
       l_rowtype.appid := UPPER(p_wdx_views_rec.appid); 
       l_rowtype.name := p_wdx_views_rec.name; 
       l_rowtype.title := p_wdx_views_rec.title; 
       l_rowtype.source := p_wdx_views_rec.source; 
       l_rowtype.description := p_wdx_views_rec.description; 
       l_rowtype.visible := p_wdx_views_rec.visible; 
       l_rowtype.created_by := p_wdx_views_rec.created_by; 
       l_rowtype.created_date := p_wdx_views_rec.created_date; 
       l_rowtype.modified_by := p_wdx_views_rec.modified_by; 
       l_rowtype.modified_date := p_wdx_views_rec.modified_date;        
       
       INSERT INTO wdx_views
         VALUES   l_rowtype;
         
               
    
    
    END ins;

    PROCEDURE upd (p_wdx_views_rec IN wdx_views_rt, p_ignore_nulls IN boolean := FALSE)
    IS
      
    BEGIN
       
      
       IF NVL (p_ignore_nulls, FALSE)
       THEN
          UPDATE   wdx_views
             SET   title = NVL(p_wdx_views_rec.title,title) ,
                   source = NVL(p_wdx_views_rec.source,source) ,
                   description = NVL(p_wdx_views_rec.description,description) ,
                   visible = NVL(p_wdx_views_rec.visible,visible) ,
                          modified_by = USER ,
                          modified_date = SYSDATE 
           WHERE  appid = UPPER(p_wdx_views_rec.appid) AND 
                  name = p_wdx_views_rec.name;
       ELSE
          UPDATE   wdx_views
             SET   title = p_wdx_views_rec.title ,
                   source = p_wdx_views_rec.source ,
                   description = p_wdx_views_rec.description ,
                   visible = p_wdx_views_rec.visible ,
                          modified_by = USER ,
                          modified_date = SYSDATE             
           WHERE appid = UPPER(p_wdx_views_rec.appid) AND 
                 name = p_wdx_views_rec.name;
       END IF;
       
       IF SQL%ROWCOUNT != 1 THEN RAISE e_upd_failed; END IF;
      

    EXCEPTION
       WHEN e_del_failed
       THEN
          raise_application_error (-20000, 'No rows were updated. The update failed.');
              
    END upd;
    
    
    PROCEDURE upd_rowid (p_wdx_views_rec IN wdx_views_rt, p_ignore_nulls IN boolean := FALSE)
    IS
      
    BEGIN
       
      
       IF NVL (p_ignore_nulls, FALSE)
       THEN
          UPDATE   wdx_views
             SET   title = NVL(p_wdx_views_rec.title,title) ,
                   source = NVL(p_wdx_views_rec.source,source) ,
                   description = NVL(p_wdx_views_rec.description,description) ,
                   visible = NVL(p_wdx_views_rec.visible,visible) ,

                          modified_by = USER ,
                          modified_date = SYSDATE 
           WHERE  ROWID = p_wdx_views_rec.row_id;
       ELSE
          UPDATE   wdx_views
             SET   title = p_wdx_views_rec.title ,
                   source = p_wdx_views_rec.source ,
                   description = p_wdx_views_rec.description ,
                   visible = p_wdx_views_rec.visible ,
                          modified_by = USER ,
                          modified_date = SYSDATE             
           WHERE  ROWID = p_wdx_views_rec.row_id;
       END IF;
       
       IF SQL%ROWCOUNT != 1 THEN RAISE e_upd_failed; END IF;
      

    EXCEPTION
       WHEN e_del_failed
       THEN
          raise_application_error (-20000, 'No rows were updated. The update failed.');
              
    END upd_rowid;

   PROCEDURE web_upd (
      p_wdx_views_rec    IN wdx_views_rt
    , p_ignore_nulls         IN boolean := FALSE
   )
   IS
      
      l_wdx_views_rec wdx_views_rt;
   BEGIN
       
       
      OPEN wdx_views_cur(UPPER(p_wdx_views_rec.appid) , p_wdx_views_rec.name);
                              
      FETCH wdx_views_cur INTO l_wdx_views_rec;

      IF wdx_views_cur%NOTFOUND THEN
         CLOSE wdx_views_cur;
         RAISE e_row_missing;
      ELSE
         IF p_wdx_views_rec.hash != l_wdx_views_rec.hash THEN
            CLOSE wdx_views_cur;
            RAISE e_ol_check_failed;
         ELSE
            IF NVL(p_ignore_nulls, FALSE)
            THEN
                UPDATE   wdx_views
                   SET   title = NVL(p_wdx_views_rec.title,title) ,
                   source = NVL(p_wdx_views_rec.source,source) ,
                   description = NVL(p_wdx_views_rec.description,description) ,
                   visible = NVL(p_wdx_views_rec.visible,visible) ,
                          modified_by = USER ,
                          modified_date = SYSDATE                                      
               WHERE CURRENT OF wdx_views_cur;
            ELSE
                UPDATE   wdx_views
                   SET   title = p_wdx_views_rec.title ,
                   source = p_wdx_views_rec.source ,
                   description = p_wdx_views_rec.description ,
                   visible = p_wdx_views_rec.visible ,
                          modified_by = USER ,
                          modified_date = SYSDATE
               WHERE CURRENT OF wdx_views_cur;
            END IF;

            CLOSE wdx_views_cur;
         END IF;
      END IF;

      

   EXCEPTION
     WHEN e_ol_check_failed
     THEN
        raise_application_error (-20000 , 'Current version of data in database has changed since last page refresh.');
     WHEN e_row_missing
     THEN
        raise_application_error (-20000 , 'Delete operation failed because the row is no longer in the database.');
     
   END web_upd;
   
   PROCEDURE web_upd_rowid (
      p_wdx_views_rec    IN wdx_views_rt
    , p_ignore_nulls         IN boolean := FALSE
   )
   IS
      
      l_wdx_views_rec wdx_views_rt;
   BEGIN
       
       
      OPEN wdx_views_rowid_cur(p_wdx_views_rec.row_id);
                              
      FETCH wdx_views_rowid_cur INTO l_wdx_views_rec;

      IF wdx_views_rowid_cur%NOTFOUND THEN
         CLOSE wdx_views_rowid_cur;
         RAISE e_row_missing;
      ELSE
         IF p_wdx_views_rec.hash != l_wdx_views_rec.hash THEN
            CLOSE wdx_views_rowid_cur;
            RAISE e_ol_check_failed;
         ELSE
            IF NVL(p_ignore_nulls, FALSE)
            THEN
                UPDATE   wdx_views
                     SET  title = NVL(p_wdx_views_rec.title,title) ,
                          source = NVL(p_wdx_views_rec.source,source) ,
                          description = NVL(p_wdx_views_rec.description,description) ,
                          visible = NVL(p_wdx_views_rec.visible,visible) ,
                          modified_by = USER ,
                          modified_date = SYSDATE                                      
               WHERE CURRENT OF wdx_views_rowid_cur;
            ELSE
                UPDATE   wdx_views
                 SET  title = p_wdx_views_rec.title ,
                      source = p_wdx_views_rec.source ,
                      description = p_wdx_views_rec.description ,
                      visible = p_wdx_views_rec.visible ,
                      modified_by = USER ,
                      modified_date = SYSDATE    
               WHERE CURRENT OF wdx_views_rowid_cur;
            END IF;

            CLOSE wdx_views_rowid_cur;
         END IF;
      END IF;

      


   EXCEPTION
     WHEN e_ol_check_failed
     THEN
        raise_application_error (-20000 , 'Current version of data in database has changed since last page refresh.');
     WHEN e_row_missing
     THEN
        raise_application_error (-20000 , 'Delete operation failed because the row is no longer in the database.');
     
   END web_upd_rowid;   

    PROCEDURE del (
        p_appid IN wdx_views.appid%TYPE,
		p_name IN wdx_views.name%TYPE
              )
    IS
      
    BEGIN
       
       
       DELETE FROM   wdx_views
             WHERE   appid = UPPER(del.p_appid) AND 
                     name = del.p_name;
       IF sql%ROWCOUNT != 1
       THEN
          RAISE e_del_failed;
       END IF;
    
          
    
    
    EXCEPTION
       WHEN e_del_failed
       THEN
          raise_application_error (-20000, 'No rows were deleted. The delete failed.');
                 
    END del;
    
    PROCEDURE del_rowid (p_rowid IN varchar2)
    IS
      
    BEGIN
       
       
       DELETE FROM   wdx_views
             WHERE   ROWID = p_rowid;
             
       IF sql%ROWCOUNT != 1
       THEN
          RAISE e_del_failed;
       END IF;
       
          
    
    EXCEPTION
       WHEN e_del_failed
       THEN
          raise_application_error (-20000, 'No rows were deleted. The delete failed.');
              
    END del_rowid;    
                        
    PROCEDURE web_del (
        p_appid IN wdx_views.appid%TYPE,
		p_name IN wdx_views.name%TYPE
      , p_hash IN varchar2
   )
   IS
      
      l_wdx_views_rec wdx_views_rt;
   BEGIN

      
       
      OPEN wdx_views_cur(UPPER(web_del.p_appid), web_del.p_name);
                              
      FETCH wdx_views_cur INTO l_wdx_views_rec;

      IF wdx_views_cur%NOTFOUND THEN
         CLOSE wdx_views_cur;
         RAISE e_row_missing;
      ELSE
         IF p_hash != l_wdx_views_rec.hash THEN
            CLOSE wdx_views_cur;
            RAISE e_ol_check_failed;
         ELSE
            DELETE FROM wdx_views
            WHERE CURRENT OF wdx_views_cur;

            CLOSE wdx_views_cur;
         END IF;
      END IF;


      

   EXCEPTION
     WHEN e_ol_check_failed
     THEN
        raise_application_error (-20000 , 'Current version of data in database has changed since last page refresh.');
     WHEN e_row_missing
     THEN
        raise_application_error (-20000 , 'Delete operation failed because the row is no longer in the database.');
       
   END web_del;

   PROCEDURE web_del_rowid (p_rowid IN varchar2, p_hash IN varchar2)
   IS
      
      l_wdx_views_rec wdx_views_rt;
   BEGIN

      
       
      OPEN wdx_views_rowid_cur(web_del_rowid.p_rowid);
                              
      FETCH wdx_views_rowid_cur INTO l_wdx_views_rec;

      IF wdx_views_rowid_cur%NOTFOUND THEN
         CLOSE wdx_views_rowid_cur;
         RAISE e_row_missing;
      ELSE
         IF web_del_rowid.p_hash != l_wdx_views_rec.hash THEN
            CLOSE wdx_views_rowid_cur;
            RAISE e_ol_check_failed;
         ELSE
            DELETE FROM wdx_views
            WHERE CURRENT OF wdx_views_rowid_cur;

            CLOSE wdx_views_rowid_cur;
         END IF;
      END IF;

      

   EXCEPTION
     WHEN e_ol_check_failed
     THEN
        raise_application_error (-20000 , 'Current version of data in database has changed since last page refresh.');
     WHEN e_row_missing
     THEN
        raise_application_error (-20000 , 'Delete operation failed because the row is no longer in the database.');
       
   END web_del_rowid;

END tapi_wdx_views;
/


