   ...
   */
   ...
   IS
      SELECT
            ...
            modified_by,
            modified_date,
            tapi_wdx_ldap.hash(appid),
            ROWID
      FROM wdx_ldap
      WHERE
           appid = wdx_ldap_cur.p_appid
      FOR UPDATE;
   ...
   END rt_for_update;
    ...
    BEGIN
       ...
       LOOP
              l_wdx_ldap_rec.appid := c1.appid;
              l_wdx_ldap_rec.ldap_name := c1.ldap_name;
              ...
              l_wdx_ldap_rec.hash := tapi_wdx_ldap.hash( c1.appid);
    ...
    IS
        ...
        l_user_name   wdx_ldap.created_by%TYPE := USER;
        l_date        wdx_ldap.created_date%TYPE := SYSDATE;

    BEGIN

        p_wdx_ldap_rec.created_by := l_user_name;
        p_wdx_ldap_rec.created_date := l_date;
        p_wdx_ldap_rec.modified_by := l_user_name;
        p_wdx_ldap_rec.modified_date := l_date;

        l_rowtype.appid := ins.p_wdx_ldap_rec.appid;
        l_rowtype.ldap_name := ins.p_wdx_ldap_rec.ldap_name;
    ...
    BEGIN
       ...
       THEN
          UPDATE   wdx_ldap
             SET appid = NVL(p_wdx_ldap_rec.appid,appid),
                ldap_name = NVL(p_wdx_ldap_rec.ldap_name,ldap_name),
           ...
           WHERE
                appid = upd.p_wdx_ldap_rec.appid
                ;
       ELSE
          UPDATE   wdx_ldap
             SET appid = p_wdx_ldap_rec.appid,
                ldap_name = p_wdx_ldap_rec.ldap_name,
